...
def march():
    ...
    while True:
        ...
        #当前位置是节点
        # brake(0.001)
        if (TrackSensorLeftValue2==False and TrackSensorRightValue1==False and(TrackSensorRightValue2==False or TrackSensorLeftValue1==False) and not currstate) or march_times==0:
            march_times+=1
            brake(0.001)
            print("当前是%d个节点"%(i))
...
def servo_color_carstate():
    ...
    time.sleep(0.8)
    rightdistance = Distance_test1()

    # 舵机旋转到180度，即左侧，测距
    servo_appointed_detection(180)
    time.sleep(0.8)
    leftdistance = Distance_test1()

    # 舵机旋转到90度，即前方，测距
    servo_appointed_detection(90)
    time.sleep(0.8)
    frontdistance = Distance_test1()
...
def pic_sb(filePath):
    ...
    options["baike_num"] = 5
    """ 带参数调用通用物体识别,返回字典 """
    images=client.advancedGeneral(image)
    per=images['result'][0]['score']*100
    key=images['result'][0]['keyword']
    print("相似度：%s%%"%per)
    print("识别结果：%s"%key)
...
try:
    ...
    while True and is_run == False:
        ...
        #以上6种电平状态时小车原地右转
        #处理右锐角和右直角的转动
        if (TrackSensorLeftValue2==False and TrackSensorRightValue1==False and(TrackSensorRightValue2==False or TrackSensorLeftValue1==False)):
            time.sleep(0.3)
            brake(1.0)
            print("red")
            break
