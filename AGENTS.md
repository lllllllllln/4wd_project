...
本文件是给 Codex 和团队成员用的资料检索入口。目标不是重复所有文档内容，而是帮助快速判断：当前问题应该先查哪个文件、为什么查它、查完后如何交叉验证。

# Project Overview

本项目是计算机工程实践项目，硬件为 Yahboom 4WD 树莓派智能小车。

最终目标是尽量复刻参考项目的能力和演示效果，但不是照抄源码。参考项目只能作为功能、流程和实现思路的来源，当前项目代码必须按本仓库的目录、接口和硬件环境重新整理。
...
- `docs/reference-replica-plan.md`
  ...
  - 适合问题：下一阶段做什么、功能优先级怎么排、某个模块应该放在哪个目录、四个人怎么分工。
  - 不适合问题：具体 GPIO 编号、厂商原始代码细节、参考项目某行代码含义。

- `docs/参考项目模块分析.md`
  - 用途：查参考项目每个模块的功能、源码位置、依赖、硬编码配置、迁移风险、是否适合优先迁移。
  - 适合问题：参考项目里循迹、避障、A*、人脸识别、全景拍摄、摩斯电码等功能在哪里；哪些模块适合先复刻。
  - 不适合问题：当前小车真实接线是否一致、当前代码是否已经实现。

建议流程：

```text
先查 reference-replica-plan.md 判断当前阶段和边界
再查 参考项目模块分析.md 找对应参考模块和风险
最后再打开原始源码或厂商资料核对细节
```
